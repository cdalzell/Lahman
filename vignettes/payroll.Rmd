...
Chapter 10 in *Data Science in R*. They use `SQL` on the raw Lahman files `.csv`, rather
than the Lahman package.
We largely use `dplyr` for data munging and `ggplot2` for plotting.
...
We are only going to use the observations from 1985, and a few variables, so we filter and select them now.
...
```{r merge-teams}
payroll <- merge(payroll, Teams[,c("yearID", "teamID","name", "WSWin")], by=c("yearID", "teamID")) 
sample_n(payroll, 10)
```
...
we would probably want to use $\log(salary)$.  We might also want to look separarately at the
